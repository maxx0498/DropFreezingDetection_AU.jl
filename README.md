...
Press ```Next``` after filling up the metadata. Now the software will parse the image and guess the location of each drop in the image. 
...
Press ```Next``` when you are happy with the drop centers. Now the software will load and tile all images, and transform the data into a stack. It will then search through the stack and identify freeze events. 
